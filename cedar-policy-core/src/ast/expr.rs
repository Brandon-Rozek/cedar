...
#[serde(into = "ExprCSE")]
#[serde(try_from = "ExprCSE")]
 pub struct Expr<T: Default + std::clone::Clone = ()> 
 ...
 pub enum ExprKind<T: Default + std::clone::Clone = ()> {
     /// Literal value
     Lit(Literal),
     /// Variable
     Var(Var),
 ...
 impl<T: Default + std::clone::Clone> Expr<T> {
 ...
 impl From<&Expr> for proto::Expr {
     fn from(v: &Expr) -> Self {
         ...
         match &v.expr_kind {
             ...
             ExprKind::Var(v) => {
                 expr_kind.ty = proto::expr::expr_kind::ExprKindType::VarTy.into();
                 expr_kind.var = proto::expr::expr_kind::Var::from(v).into();
             }
             ExprKind::Slot(sid) => {
                 expr_kind.ty = proto::expr::expr_kind::ExprKindType::Slot.into();
                 expr_kind.slot = proto::SlotId::from(sid).into();
             }
             ExprKind::Unknown(u) => {
                 expr_kind.ty = proto::expr::expr_kind::ExprKindType::UnknownTy.into();
                 expr_kind.unknown = Some(proto::expr::expr_kind::Unknown::from(u));
             }
 ...
 impl<T: Default + std::clone::Clone> Default for ExprBuilder<T> {
     fn default() -> Self {
         Self::new()
     }
 }
 
 impl<T: Default + std::clone::Clone> ExprBuilder<T> {
     ...
     pub fn with_same_source_loc<U: Default + std::clone::Clone>(self, expr: &Expr<U>) -> Self {
 ...
 impl<T: Default + Clone> ExprBuilder<T> {
 ...
 pub struct ExprShapeOnly<'a, T: Default + std::clone::Clone = ()>(&'a Expr<T>);
 
 impl<'a, T: Default + std::clone::Clone> ExprShapeOnly<'a, T> {
 ...
 impl<'a, T: Default + std::clone::Clone> PartialEq for ExprShapeOnly<'a, T> {
     fn eq(&self, other: &Self) -> bool {
         self.0.eq_shape(other.0)
     }
 }
 
 impl<'a, T: Default + std::clone::Clone> Hash for ExprShapeOnly<'a, T> {
 ...
 impl<T: Default + std::clone::Clone> Expr<T> {
     ...
     /// data were added.
     pub fn eq_shape<U: Default + std::clone::Clone>(&self, other: &Expr<U>) -> bool {
         use ExprKind::*;
 ...
 impl From<&proto::expr::expr_kind::Var> for Var {
     fn from(v: &proto::expr::expr_kind::Var) -> Self {
         match v {
             ...
             proto::expr::expr_kind::Var::Resource => Var::Resource,
             proto::expr::expr_kind::Var::Context => Var::Context
         }
     }
 }
 ...
 impl From<&Var> for proto::expr::expr_kind::Var {
     fn from(v: &Var) -> Self {
         match v {
             ...
             Var::Resource => proto::expr::expr_kind::Var::Resource,
             Var::Context => proto::expr::expr_kind::Var::Context
         }
     }
 }
